        ...
        public async Task<string?> ConfirmOutcomeAsync(string extractedBetDataJson, DateTime betPlacedAt, int? betId = null)
        {
            ...
            try
            {
                ...
                var prompt = $"You are a sports betting expert. Here is the JSON data of a bet slip that was placed at {betPlacedAt:yyyy-MM-dd HH:mm}.\n" +
                             ...
                             $"CRITICAL FOR FALLBACK / GOOGLE SEARCH: If the FotMob API does not provide the specific data needed to grade the bet (e.g. you need corners/cards but the API only provides basic scores), OR if there is no FotMob JSON provided at all for a match, you are EQUIPPED WITH A GOOGLE SEARCH TOOL. You MUST use Google Search to find the final missing result (e.g. check ESPN, Flashscore, or official league sites). DO NOT use Google Search if the FotMob JSON already contains the necessary data to grade the bet (e.g. you only need the final score and the JSON provides it). Do not guess or hallucinate stats. WHEN USING GOOGLE SEARCH, YOU MUST VERIFY THE DATE OF THE MATCH! It is extremely common for teams/players to have played each other in the past. You must ensure the match you found occurred on or near the correct date as listed in the bet slip's StartTime. If no explicit StartTime is given for the leg, you must ensure you are fetching the MOST RECENT match played. If you find a match from months or years ago, DO NOT use it; mark the leg as 'UNKNOWN' instead. If you still cannot find the correct match online, mark the leg as 'UNKNOWN' or 'Pending'.\n" +

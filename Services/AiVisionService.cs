        ...
        public async Task<string?> ConfirmOutcomeAsync(string extractedBetDataJson, DateTime betPlacedAt, DateTime? matchStartTime = null, int? betId = null)
        {
            ...
            try
            {
                ...
                var prompt = $"You are a sports betting expert. Here is the JSON data of a bet slip that was placed at {betPlacedAt:yyyy-MM-dd HH:mm}.\n" +
                             ...
                             $"CRITICAL FOR API VERIFICATION: In the `fullAnalysis` field, the VERY FIRST line MUST explicitly state whether API JSON data (FotMob) was successfully provided and what it contained. E.g., 'API Status: Match found, but no scores available yet' or 'API Status: Scores found (1-0)'. If the API lacked data (e.g., corners are missing or no scores found), do NOT guess or hallucinate stats. You must strictly use the data provided or fallback to Google Search.\n" +
                             ...
                             $"CRITICAL FOR SCREENSHOTS: I will attach the RAW JSON statistics fetched directly from our APIs (FotMob) for the matches in this bet slip if available. You MUST carefully parse this JSON to find the exact scores for the specific teams requested in the bet legs! This JSON data is your absolute primary source of truth.\n" +
                             ...
                             $"1. THE EXACT TIMEFRAME: For each leg, check if a 'startTime' is already provided in the JSON data. If a leg has a 'startTime', you MUST find the match that corresponds to that EXACT UTC date and time! If a leg does NOT have a 'startTime', the exact match time is unknown, but this bet slip was uploaded at {betPlacedAt:yyyy-MM-dd HH:mm} UTC. For those legs, you MUST find the VERY FIRST match played chronologically ON OR AFTER this upload time.\n" +
                             ...
                             $"CRITICAL FOR SOURCES: For every match, explicitly state your verification source at the very beginning of your 'stats' text. You MUST start your stats description with EXACTLY ONE of the following three phrases:\n" +
                             $"- 'Verified via FotMob: ' (Use this if you found all necessary stats within the provided FotMob JSON block)\n" +
                             $"- 'FotMob lacked stat; Verified via Google Search: ' (Use this if FotMob JSON was provided but lacked the specific stat needed, forcing you to use Google Search)\n" +
                             $"- 'Verified via Google Search: ' (Use this if absolutely no JSON was provided and you had to rely entirely on Google Search)\n" +
                             $"If you use Google Search for any reason, you MUST include exactly ONE URL to the specific source page you used (e.g. https://www.espn.com/tennis/match/_/id/123456).\n" +
                             $"CRITICAL FOR GOOGLE SEARCHES: When using Google Search for ANY sport, DO NOT rely on or search for aggregator sites like FlashScore, LiveScore, or SofaScore, as their search snippets lack the detailed stats required for grading. You MUST prioritize searching official websites (like ATPTour.com, ESPN.com, HLTV.org) or primary sources for comprehensive match stats!\\n" +
                             $"CRITICAL TO AVOID PREDICTIONS: When using Google Search, you MUST NEVER use betting prediction articles, match previews, or odds comparison sites (e.g. lines.com, actionnetwork) as your source! These articles are written BEFORE the match and often contain hypothetical scenarios that look like final results. You MUST verify that the page you are reading is a POST-MATCH report containing the ACTUAL FINAL SCORES.\\n" +
                             $"CRITICAL FOR PLAYER PROPS (STARTER RULE & VOIDS): ALL player proposition bets (e.g., Goalscorer, Player to be Carded, Shots) have strict void rules! Rule 1: If a player NEVER steps on the pitch (0 minutes played, not in squad, or left on bench), the outcome MUST ALWAYS BE MARKED AS 'Void', regardless of whether it's a live bet or a Power Sub! Rule 2: If the bet slip is NOT marked as `isLive: true`, the player MUST be in the STARTING XI. If they do not start (even if they are subbed on later), it MUST be marked as 'Void'. Rule 3: If `isLive` is true, the starter rule does not apply; if they are subbed on later and play, the bet stands. Rule 4: 'Power Sub' bets ONLY activate if the named player STARTS the match. If the named player does not start, the Power Sub bet is 'Void' (the substitute is ignored) UNLESS `isLive` is true and the player eventually steps on the pitch (in which case the bet stands as a normal player prop).\n" +
                             $"CRITICAL FOR POWER SUB: If the selection contains '(Power Sub)' (or 'Super Sub'), the bet transfers to the substitute ONLY IF the named player started the match! If they started and are substituted off, the stats of the player who comes on for them MUST be added to their total! If the named player did not start, you DO NOT look at substitutes; evaluate them strictly under the Player Prop rules above. To do this, look at the FotMob 'events' array for a 'Substitution' event where 'swap' contains the named player. The other player in the 'swap' array is the substitute. Find both players in 'playerStats' and mathematically add their stats together.\n" +
                             $"CRITICAL FOR '1' AND '2' IN MARKETS: In betting, '1' often means Home Team and '2' means Away Team. BUT, if a market is extracted as 'Total Goals 1' or 'Total Goals 2', and the selection is an Over/Under line (e.g. 'Over 1.0', 'Over 2.0'), you MUST strongly consider if the '1' or '2' is actually an accidental truncation of the Asian Line (e.g. '1.0' or '2.0') for the entire match, rather than a Team ID. If the slip does not explicitly state a specific team's name, treat it as the TOTAL match goals for both teams combined!\n" +
                             ...
                             $"CRITICAL FOR EXACT ASIAN LINES (.0): Any selection line ending in exactly '.0', '.25', or '.75' (e.g. 'Over 5.0', 'Under 3.0', '-2.0', '+1.0', '2.25', '1.75') is fundamentally an ASIAN Handicap or ASIAN Total! For Asian lines ending in exactly '.0', if the final result lands EXACTLY on the line (e.g., 'Over 5.0' and exactly 5 corners occurred), the outcome MUST be marked as 'Void' (Refund). NEVER mark an exact '.0' tie as 'Lost'. The ONLY exception to this rule is if the market explicitly says '3-way' (e.g., '3-way Handicap -2.0'), in which case it is assessed as a standard 3-way market without refunds.\n" +
                             ...
                             $"CRITICAL FOR SCHEDULING: If the match has NOT STARTED (e.g. `general.started` is false AND `header.status.started` is false), you MUST NOT grade ANY legs as Won or Lost; all legs must be 'Pending'. You must also determine its exact kickoff time in UTC. If the kickoff time is ALREADY clearly stated in the bet slip data (e.g. 'Starts: 28.Jul 11:00'), you MUST parse it directly and DO NOT use Google Search. Only use Google Search if the start time is missing. CRITICAL: For 'matchStartTimeIso', you MUST return the absolute EARLIEST start time across all PENDING or UNFINISHED legs in the entire bet slip (e.g. \"2026-07-25T19:00:00Z\"). Do NOT include the start times of legs that have already finished.\n" +

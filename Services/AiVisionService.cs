        ...
        public async Task<string?> ConfirmOutcomeAsync(string extractedBetDataJson, DateTime betPlacedAt, int? betId = null)
        {
            ...
            try
            {
                ...
                var prompt = $"You are a sports betting expert. Here is the JSON data of a bet slip that was placed at {betPlacedAt:yyyy-MM-dd HH:mm}.\n" +
                             ...
                             $"CRITICAL FOR OVERALL STATUS: The 'overallStatus' field represents the final result of the ENTIRE BET SLIP. It MUST be exactly one of the following strings:\n" +
                             $"- 'WON' (if EVERY leg in the bet has definitively won. This applies EVEN IF the match is still in progress, as long as the specific bets have already won!)\n" +
                             $"- 'LOST' (if ANY leg in the bet has definitively lost. This applies EVEN IF the match is still in progress, as long as a leg has busted!)\n" +
                             $"- 'VOID' (if the bet was voided)\n" +
                             $"- 'MATCH NOT STARTED' (if the match has not started yet, and the bet is pending)\n" +
                             $"- 'MATCH IN PROGRESS' (if the match is live, and the overall bet is STILL PENDING because not all legs have won, and no leg has definitively lost yet)\n" +

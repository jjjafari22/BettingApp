        ...
        public async Task<string?> ConfirmOutcomeAsync(string extractedBetDataJson, DateTime betPlacedAt)
        {
            ...
            try
            {
                var prompt = $"You are a sports betting expert. Here is the JSON data of a bet slip that was placed at {betPlacedAt:yyyy-MM-dd HH:mm}.\n" +
                             ...
                             $"CRITICAL FOR CORNERS, CARDS, AND PLAYER PROPS: These specific statistics are notoriously difficult to find accurately in standard Google snippets. You MUST explicitly verify these stats on highly reliable sources like UEFA, FIFA, Flashscore, Sofascore, ESPN, or BBC. Do NOT guess or hallucinate stats from unrelated summaries! If the match is FINISHED but you cannot find the EXACT definitive final count for corners or cards, mark the bet as 'UNKNOWN' rather than guessing a wrong number. Note: Do NOT use UNKNOWN if the match simply hasn't started yet!\n" +
                             $"CRITICAL FOR SOURCES: For every match, you MUST include the EXACT URL link of the website you used to verify the stats directly in the 'stats' field or 'fullAnalysis' field so the admin can verify it! Never claim 'according to UEFA' without providing the uefa.com link!\n" +
